...
def shift_pitch(
    ...
    try:
        ...
        raise ImportError(
            "librosa is required for pitch shifting. "
...
def stretch_time(
    ...
    try:
        ...
        raise ImportError(
            "librosa is required for time stretching. "
            "Install with: pip install librosa"
        ) from e
...
def apply_voice_effect(
    ...
    # Build FFmpeg filter based on effect and intensity
    filter_chain = _build_voice_effect_filter(effect, intensity)
...
def _build_voice_effect_filter(effect: str, intensity: float) -> str:
    ...
    if effect == "robot":
        ...
        # Use asetrate to lower pitch (0.85-0.95 of original)
        rate_factor = 0.95 - i * 0.1
        return f"asetrate=44100*{rate_factor},aresample=44100,bass=g={bass_boost}"

    elif effect == "chipmunk":
        # Higher pitched, faster voice
        # Use asetrate to raise pitch (1.2-1.6 of original)
        rate_factor = 1.2 + i * 0.4
        return f"asetrate=44100*{rate_factor},aresample=44100"
